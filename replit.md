...
- **Aggregate `/api/regions` endpoint:** New route in `artifacts/api-server/src/routes/regions.ts` with a 5-minute in-memory cache. Aggregates Open-Meteo headline + 6-day daily forecast for Snowy Mountains (Thredbo Top, 1957m) and Yamanouchi (Shiga Kogen, 1800m). Constructs proper ISO UTC `observedAt` from Open-Meteo's `utc_offset_seconds`. NaN/missing data is null-guarded; if upstream returns no temperature, the cache is skipped.
