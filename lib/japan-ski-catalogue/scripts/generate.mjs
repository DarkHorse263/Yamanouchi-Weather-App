...
function validateLatestPublicationBatchGeography({
  manifestIds,
  batchStarts,
  ...
  const latestBatchIds = new Set(manifestIds.slice(batchStarts.at(-1)));
  const intakeById = new Map(intakeRecords.map((record) => [record.recordId, record]));
  const candidatesByTown = new Map();
  for (const candidate of publicationCandidates) {
    const intake = intakeById.get(candidate.recordId);
    if (!intake) continue;
    const key = `${intake.raw.prefecture}\u0000${candidate.baseTownId}`;
    candidatesByTown.set(key, [...(candidatesByTown.get(key) ?? []), candidate]);
  }
  const anchorFor = ([key, candidates], excludedRecordId) => {
    const peers = candidates.filter((candidate) => candidate.recordId !== excludedRecordId);
    if (peers.length === 0) return undefined;
    const [prefecture, baseTownId] = key.split("\u0000");
    return {
      prefecture,
      baseTownId,
      coordinates: {
        lat:
          peers.reduce((total, candidate) => total + candidate.coordinates.lat, 0) /
          peers.length,
        lng:
          peers.reduce((total, candidate) => total + candidate.coordinates.lng, 0) /
          peers.length,
      },
    };
  };
  const implausible = [];
  for (const candidate of publicationCandidates) {
    if (!latestBatchIds.has(candidate.recordId)) continue;
    const intake = intakeById.get(candidate.recordId);
    if (!intake) continue;
    const independentAnchors = [...candidatesByTown.entries()]
      .map((entry) => anchorFor(entry, candidate.recordId))
      .filter(Boolean);
    const assignedAnchor = independentAnchors.find(
      (anchor) =>
        anchor.prefecture === intake.raw.prefecture &&
        anchor.baseTownId === candidate.baseTownId,
    );
    const nearestAlternative = independentAnchors
      .filter(
        (anchor) =>
          anchor.prefecture === intake.raw.prefecture &&
          anchor.baseTownId !== candidate.baseTownId,
      ...
      .map((anchor) => ({
        ...anchor,
        distanceKm: haversineKm(candidate.coordinates, anchor.coordinates),
      }))
      .sort((left, right) => left.distanceKm - right.distanceKm)[0];
    const assignment = assignments.get(candidate.recordId);
    if (assignment?.basis === "publication_geography_audit") {
      continue;
    }
    if (!assignedAnchor) {
      if (
        nearestAlternative?.distanceKm < 30
      ) {
        implausible.push(
          `${candidate.recordId}: ${candidate.baseTownId} has no independent peer anchor while ${nearestAlternative.baseTownId} is ${nearestAlternative.distanceKm.toFixed(1)} km away; add authoritative municipality review before publication`,
        );
      }
      continue;
    }
    const assignedDistanceKm = haversineKm(candidate.coordinates, assignedAnchor.coordinates);
    if (
      assignedDistanceKm > 75 &&
      nearestAlternative &&
      nearestAlternative.distanceKm + 20 < assignedDistanceKm &&
      nearestAlternative.distanceKm * 1.5 < assignedDistanceKm
    ) {
      implausible.push(
        `${candidate.recordId}: ${candidate.baseTownId} is ${assignedDistanceKm.toFixed(1)} km from its peer anchor; ${nearestAlternative.baseTownId} is ${nearestAlternative.distanceKm.toFixed(1)} km away`,
      );
    }
  }
  if (implausible.length > 0) {
    throw new Error(
      `Latest publication batch contains implausible geographic assignments:\n${implausible.join("\n")}`,
    );
  }
}
...
function resolvedEvidence(directory, supplement, review) {
  ...
  if (supplement?.fields?.defensibleElevation) {
  ...
  if (supplement?.fields?.coordinates) {
...
function validateFoundationSources({ reviewSource, travelClusters, supplements, publicIdentities }) {
  ...
  for (const supplement of supplements.records ?? []) {
    ...
    const unresolvedFields = supplement.unresolved ?? [];
    check(
      unresolvedFields.every((field) => ["japaneseName", "officialUrl", "defensibleElevation", "coordinates"].includes(field)),
      `Unsupported unresolved supplement field: ${supplement.recordId}`,
    );
    check(
      new Set(unresolvedFields).size === unresolvedFields.length,
      `Duplicate unresolved supplement field: ${supplement.recordId}`,
    );
    for (const [field, citation] of Object.entries(supplement.fields ?? {})) {
      ...
      check(
        ...
        `Supplement field is both accepted and unresolved: ${supplement.recordId}:${field}`,
      );
    }
    if (unresolvedFields.length > 0) {
      check(
        typeof supplement.notes === "string" && supplement.notes.length > 0,
        `Missing unresolved supplement note: ${supplement.recordId}`,
      );
      check(
        Array.isArray(supplement.sourcesChecked) && supplement.sourcesChecked.length > 0,
        `Missing unresolved source audit: ${supplement.recordId}`,
      );
    }
    for (const source of supplement.sourcesChecked ?? []) {
      check(isHttpUrl(source?.sourceUrl), `Missing checked-source URL: ${supplement.recordId}`);
      check(
        /^\d{4}-\d{2}-\d{2}$/.test(source?.evidenceAsOf ?? ""),
        `Missing checked-source date: ${supplement.recordId}`,
      );
      check(
        typeof source?.sourceType === "string" &&
          source.sourceType.length > 0 &&
          typeof source?.note === "string" &&
          source.note.length > 0,
        `Incomplete checked-source citation: ${supplement.recordId}`,
      );
    }
  }
...
async function buildCatalogue() {
  ...
  const readinessFieldByMissingReason = {
    missing_coordinates: "coordinates",
    missing_defensible_elevation: "defensibleElevation",
    missing_japanese_name: "japaneseName",
    missing_official_url: "officialUrl",
  };
  for (const record of intakeRecords) {
    if (record.review?.reviewStatus !== "verified_operating" || record.readiness.evidenceComplete) continue;
    const supplement = foundation.supplementById.get(record.recordId);
    if (!supplement) {
      throw new Error(`Verified essential-gap record lacks a reviewed evidence supplement: ${record.recordId}`);
    }
    const expectedUnresolved = record.readiness.missingReasons
      .map((reason) => readinessFieldByMissingReason[reason])
      .filter(Boolean)
      .sort();
    const recordedUnresolved = [...(supplement.unresolved ?? [])].sort();
    if (JSON.stringify(recordedUnresolved) !== JSON.stringify(expectedUnresolved)) {
      throw new Error(
        `Verified essential-gap review is stale: ${record.recordId}; expected unresolved ` +
          `${expectedUnresolved.join(", ")}, recorded ${recordedUnresolved.join(", ")}`,
      );
    }
  }
  ...
  if (
    ...
    publicationManifest.approvedAt !== "2026-08-27" ||
  ...
  validateLatestPublicationBatchGeography({
    manifestIds,
    batchStarts,
    publicationCandidates,
    intakeRecords,
    assignments: foundation.assignments,
  });
  const intakeById = new Map(intakeRecords.map((record) => [record.recordId, record]));
