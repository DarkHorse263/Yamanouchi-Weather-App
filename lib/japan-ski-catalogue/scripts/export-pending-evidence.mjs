...
const expectedUnclearCount = catalogue.policy.cleanGate.unclearOrNotVerifiableRecords;
const expectedEssentialGapCount = catalogue.policy.cleanGate.verifiedEssentialGapRecords;
const expectedTotal = catalogue.policy.cleanGate.reviewQueueExcludedPendingEvidence;
if (
  records.length !== expectedTotal ||
  unclearCount !== expectedUnclearCount ||
  essentialGapCount !== expectedEssentialGapCount
) {
  throw new Error(
    `Pending-evidence export disagrees with the catalogue honesty gate: expected ${expectedTotal} total ` +
      `(${expectedUnclearCount} unclear, ${expectedEssentialGapCount} essential-gap); found ${records.length} ` +
      `(${unclearCount} unclear, ${essentialGapCount} essential-gap)`,
  );
}
