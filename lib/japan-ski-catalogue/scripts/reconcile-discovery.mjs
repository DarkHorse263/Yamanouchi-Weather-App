...
function dispositionFor(record, pages) {
  ...
  if (record?.review?.reviewStatus === "verified_operating") {
    return record.readiness?.evidenceComplete ? "genuinely_missing" : "unresolved";
  }
  return "unresolved";
}
...
for (const area of reconciliation.areas) {
  ...
  const record = intakeById.get(area.identityId);
  if (
    area.operatingEvidence.status === "verified_operating" &&
    record?.readiness?.evidenceComplete &&
    area.publicCoverage.routes.length === 0
  ) {
    validationErrors.push(`Verified-operating area has no published coverage: ${area.identityId}`);
  }
