...
check(currentVerified.length === 320, `Expected 320 current verified records; found ${currentVerified.length}`);
check(mappedExisting.length === 56, `Expected 56 existing-page mappings; found ${mappedExisting.length}`);
check(newIdentities.length === 264, `Expected 264 new-page identities; found ${newIdentities.length}`);
...
check(
  ...
  "Publication manifest cannot exceed the current route-ready candidate set",
);
const eligibleVerifiedCoverage = evidenceCompleteVerifiedOperating;
const eligiblePublishedCoverage = eligibleVerifiedCoverage.filter(
  (record) => record.lifecycle === "published",
);
const reviewQueueExcludedPendingEvidence =
  unresolvedEssentialGaps.length + (reviewStatusCounts.unclear ?? 0);
const ineligiblePublishedIntake = catalogue.intakeRecords.filter(
  (record) =>
...
check(
  catalogue.policy.launchPolicyId === "verified_evidence_complete_only",
  "Unexpected public launch policy",
);
check(
  catalogue.policy.cleanGate.verifiedEvidenceCompleteRecords === eligibleVerifiedCoverage.length,
  "Verified evidence-complete policy count is stale",
);
check(
  catalogue.policy.cleanGate.lifecyclePublishedRecords === eligiblePublishedCoverage.length,
  "Published verified-only policy count is stale",
);
check(
  catalogue.policy.cleanGate.routeReadyNewCandidates === publicationReady.length,
  "Route-ready policy count is stale",
);
check(
  catalogue.policy.cleanGate.manifestIntegratedNewRecords === catalogue.publishedCatalogueRecords.length,
  "Manifest-integrated policy count is stale",
);
check(
  catalogue.policy.cleanGate.reviewQueueExcludedPendingEvidence === reviewQueueExcludedPendingEvidence,
  "Review-queue policy count is stale",
);
check(
  catalogue.policy.cleanGate.unclearOrNotVerifiableRecords === (reviewStatusCounts.unclear ?? 0),
  "Unclear/not-verifiable policy count is stale",
);
check(
  catalogue.policy.cleanGate.verifiedEssentialGapRecords === unresolvedEssentialGaps.length,
  "Verified essential-gap policy count is stale",
);
const verifiedOnlyGatePasses =
  eligiblePublishedCoverage.length === eligibleVerifiedCoverage.length &&
  publicationReady.length === catalogue.publishedCatalogueRecords.length &&
  catalogue.publishedCatalogueRecords.every((record) => record.honesty.runtimeIntegrated) &&
  ineligiblePublishedIntake.length === 0;
check(
  catalogue.policy.marketingApproved === verifiedOnlyGatePasses,
  "marketingApproved does not match the verified-only publication gate",
);
if (catalogue.policy.marketingApproved) {
  check(
    eligiblePublishedCoverage.length === eligibleVerifiedCoverage.length,
    "Marketing cannot be approved with an unpublished evidence-complete verified record",
  );
  check(ineligiblePublishedIntake.length === 0, "Marketing cannot be approved with an ineligible public intake record");
  check(
    publicationReady.length === catalogue.publishedCatalogueRecords.length,
    "Marketing cannot be approved with an unapproved route-ready new candidate",
  );
}
...
for (const record of catalogue.intakeRecords) {
  ...
  if (
    ...
    (record.review?.reviewStatus === "verified_operating" && !record.readiness.evidenceComplete)
  )
    totals.unresolved += 1;
...
function reportMarkdown() {
  ...
  const lines = [
    ...
    `- Launch policy: \`${catalogue.policy.launchPolicyId}\``,
    `- ${catalogue.policy.launchPolicyDescription}`,
    `- ${catalogue.policy.marketingGateReason}`,
    `- Verified-only published coverage: ${eligiblePublishedCoverage.length} of ${eligibleVerifiedCoverage.length}`,
    `- Review queue excluded pending evidence: ${reviewQueueExcludedPendingEvidence}`,
    `- Unclear/not-verifiable exclusions: ${reviewStatusCounts.unclear ?? 0}`,
    `- Verified essential-gap exclusions: ${unresolvedEssentialGaps.length}`,
    "",
...
if (errors.length > 0) {
  ...
  process.stdout.write(
    `Japan catalogue valid: ${catalogue.intakeRecords.length} intake rows, ` +
      ...
      `${reviewQueueExcludedPendingEvidence} review-queue exclusions; ` +
      `marketingApproved=${catalogue.policy.marketingApproved}.\n`,
  );
}