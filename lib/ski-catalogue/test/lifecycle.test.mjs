...
test("public projection omits lifecycle and evidence internals", () => {
  ...
  assert.equal("baseElevationM" in projection, false);
...
test("public projection emits an exactly cited base elevation", () => {
  const record = structuredClone(complete);
  record.evidence[0].fields.push({
    field: "baseElevation",
    value: 2200,
    citation: "Test Area has an officially published base elevation of 2200 m.",
  });
  assert.deepEqual(validateRecord(record), []);
  assert.equal(publicProjection(record).baseElevationM, 2200);
});

test("base elevation evidence fails closed when mismatched or invalid", () => {
  const mismatched = structuredClone(complete);
  mismatched.evidence[0].fields.push({
    field: "baseElevation",
    value: 2199,
    citation: "Test Area has an officially published base elevation of 2199 m.",
  });
  assert.match(validateRecord(mismatched).join("\n"), /does not match baseElevation/);
  assert.equal("baseElevationM" in publicProjection(mismatched), false);
  ...
  assert.match(validateRecord(invalid).join("\n"), /evidence URL must be an absolute HTTP URL/);
  assert.equal("baseElevationM" in publicProjection(invalid), false);

  const missingRetrievedAt = structuredClone(complete);
  missingRetrievedAt.evidence.push({
    url: "https://example.test/base-facts",
    fields: [{
      field: "baseElevation",
      value: 2200,
      citation: "Test Area has an officially published base elevation of 2200 m.",
  ...
  assert.match(validateRecord(missingRetrievedAt).join("\n"), /evidence retrievedAt is required/);
  assert.equal("baseElevationM" in publicProjection(missingRetrievedAt), false);
});

test("public projection omits an unknown base elevation", () => {
  const record = structuredClone(complete);
  record.elevations.baseM = null;
...
test("New Zealand evidence table publishes only the directed facilities", async () => {
  ...
  for (const record of published) {
    ...
    for (const field of ["identity", "status", "coordinates", "forecastElevation", "locality", "officialUrl", "baseElevation"]) {
    ...
    assert.equal(record.evidenceTable.fieldCitations.baseElevation.value, record.elevations.baseM);
    assert.equal(publicProjection(record).baseElevationM, record.elevations.baseM);
