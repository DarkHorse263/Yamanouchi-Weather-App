...
**Moving a location between regions** (e.g. Sun Peaks Powder Highway→Okanagan, Aug 2026): same registry list applies — region .ts files, api lib/regions LOCATION_TO_REGION, routes/regions summaries, roads.ts region branch, app.ts town map, RadarMap pins+center, CountryPicker fallback row, TownHome MOUNTAIN_TINTS, RegionSources, seo-regions.mjs — then regenerate sitemap (`node scripts/generate-sitemap.mjs`) AND rewrites (`node scripts/generate-rewrites.mjs`) and splice the rewrites block into artifact.toml via verifyAndReplaceArtifactToml. weather.ts LOCATIONS rows are country-tagged, only their section/comment moves.
...
**Bulk-import hazard (Aug 2026):** when a whole country lands via task agents, `routes/regions.ts` REGIONS (the /api/regions home-card catalog) is the registry most likely to be left partial — US shipped with 20 of 81 entries. After any bulk region import, compare its per-country count against `lib/regions.ts` REGION_IDS.
