...
## One per-lift surface — applies to LIVE resorts too (Aug 2026)
On the Snowy LocationDetail page, the free "On the snow" card (live resorts, i.e. Thredbo) shows ONLY headline counts (lifts/runs open) + the official lift report link — never its own per-lift rows. The single per-lift surface is LiftWindHoldPanel (premium-gated), which merges live feed statuses as chips. **Why:** the live-feed merge briefly produced TWO lift lists on Thredbo (owner-reported); the "one lift surface" rule previously only covered no-feed resorts. **How to apply:** any new live-feed resort must not reintroduce a per-lift list in the free card; pass liftReportUrl to the panel unconditionally (it only renders in the non-operating banner). Panel rows show the wind chip on mobile when there's no live chip (bare black lift names read as "saying nothing" — owner-reported on Perisher).
