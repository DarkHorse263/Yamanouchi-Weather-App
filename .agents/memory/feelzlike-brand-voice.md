...
- The middot must be the LITERAL `·` character (or `&middot;`) in JSX **attribute** strings (e.g. `PageMeta title="feelzlike · ..."`). Never the JS escape `\u00b7` in a double-quoted JSX attribute: JSX attributes are not JS string literals, so `\u00b7` is passed through verbatim and shows up as raw `\u00b7` in `og:title`/meta tags (search + social previews). `\u00b7` is only safe inside `{ }` expressions / template literals.
