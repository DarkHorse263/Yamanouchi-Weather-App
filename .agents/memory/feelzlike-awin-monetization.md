...
## trivago — JP metasearch, region-keyed, earns via Awin (NOT CJ)
...
- **Earns via Awin Convert-a-Link on the `trivago.jp` domain** (the owner's approved trivago Japan programme), so NO URL affiliate param. `cjLinkFor("trivago", …)` returns null (not a CJ advertiser) → the plain `trivago.jp` link is served → MasterTag rewrites it. Same Awin-side-approval + `canUseAds` gates as the OTAs.
- **First REGION-keyed platform.** `platformDeepLink` gained `opts.region`; trivago returns `TRIVAGO_DESTINATIONS[region] ?? ""`. An unmapped region returns `""`, and consumers DROP empty links (no dead button): `StayPlatformBar` builds a filtered `links` array; `TownStay` builds filtered `stayLinks`. **To add a new JP region's trivago button you MUST add its verified locid to `TRIVAGO_DESTINATIONS`** — otherwise it auto-hides (correct, not a bug).
