...
- Owner feedback baked in (v3): people DRIVE to snow from town (no snow at the door/window), nobody talks on camera (generated lips can't sync to TTS — narration only), no "free until" line in video ads. App-on-screen shots are HONEST: generate a locked-off clip of a phone with a plain WHITE screen, then corner-pin the real screenshot onto it (flood-fill the white region from a centre seed — extreme-point corner picking gets poisoned by lamp glow; clips are 720x1280 native; PIL PERSPECTIVE warp → full-frame RGBA overlay → ffmpeg overlay).
