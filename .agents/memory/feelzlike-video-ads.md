...
- Headless chromium has NO CJK fonts by default — Japanese UI renders as tofu boxes. Install noto-fonts-cjk-sans (nix), point FONTCONFIG_FILE at a fonts.conf listing the nix-store fonts dir, pass env to puppeteer.launch.
...
- v3 anthem cuts are country-specific (AU=AU+NZ screens, US=US+CA, JP=JP) with the coverage sentence reordered per market; the alerts subscribe form is season-gated so in southern winter only AU/NZ pages show it — for other markets scroll the same form to that country's region checkboxes.
- Size each cut to its VO (detect the closing line via silencedetect, start the end card just before it, stretch/squeeze segments with setpts) — never pad one fixed-length video with delayed VO tails, that's what caused the "huge pause" complaint.
