...
**How to apply:** any new temp/snow readout — convert with `u.temp`/`u.snow`/`u.snowVal` and label with `u.tempUnit`/`u.snowUnit` (reviews reject bare `°` without a C/F label). Wind and elevation now flip too: `u.wind`/`u.windUnit` (km/h→mph) and `u.elev`/`u.elevUnit` (m→ft); dashboard-lib components take `formatWind`/`windUnitLabel`/`formatElevation`/`elevationUnitLabel` props. Prediction copy that embeds "NNkm/h" in strings (wind-hold reasons) is rewritten at the display edge (`localiseWindCopy` in LiftWindHoldPanel). Unit-thresholds explainer copy (powder window criteria) must also be unit-aware.
