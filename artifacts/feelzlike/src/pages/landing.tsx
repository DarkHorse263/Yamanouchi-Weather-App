        ...
        <div className="space-y-3">
          {filtered.map((region, i) => {
            ...
            return (
              ...
              >
                <Wrapper {...wrapperProps} className="block w-full text-left group">
                  <div className={`flex bg-white rounded-xl border border-slate-200 overflow-hidden transition-all duration-200 ${isLive ? "hover:shadow-md hover:border-blue-300 cursor-pointer" : "opacity-90 cursor-default"}`}>
                    <div className="relative w-28 md:w-36 shrink-0 overflow-hidden">
                      <img
                        src={region.image}
                        alt={region.name}
                        className={`w-full h-full object-cover ${isLive ? "group-hover:scale-105" : ""} transition-transform duration-500`}
                      />
                      {!isLive && (
