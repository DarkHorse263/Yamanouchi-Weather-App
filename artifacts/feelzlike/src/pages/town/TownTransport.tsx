  ...
  return (
    <div className="px-4 md:px-10 py-4 md:py-8 max-w-6xl mx-auto">
      ...
      {providers.length === 0 ? (
        <EmptyStateCard
          icon={Bus}
          title={t("No scheduled transport yet", "公共交通機関の情報は準備中")}
          body={t(
            `Car hire is available above. We're still verifying buses, shuttles and rail serving ${town?.name ?? region.name} before we list them.`,
            `レンタカーは上記からご利用いただけます。${town ? t(town.name, town.nameJa) : region.name}を発着するバス・送迎・電車は確認中です。`,
          )}
        />
      ) : (
