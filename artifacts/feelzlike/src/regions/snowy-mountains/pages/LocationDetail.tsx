...
export default function LocationDetail() {
  ...
  const canvasClass = `min-h-[100dvh] ${isSummer ? "bg-[#059669]" : "bg-[#0055FF]"} pb-8 transition-colors duration-500`;

  if (weatherLoading)
    return (
      <div className={canvasClass}>
        <LoadingState message="Reading live conditions…" onCanvas />
      </div>
    );
  if (weatherError || !weatherData)
    return (
      <div className={canvasClass}>
        <ErrorState error={weatherError} onRetry={() => weatherRefetch()} onCanvas />
      </div>
    );
      ...
      <section className="relative overflow-hidden isolate">
        ...
        <div className="pointer-events-none absolute inset-0 -z-10">
          ...
          <div
            ...
            style={{
              backgroundImage:
                `linear-gradient(to bottom, transparent 65%, ${isSummer ? "#059669" : "#0055FF"} 100%), repeating-linear-gradient(0deg, hsla(0,0%,100%,0.04) 0px, hsla(0,0%,100%,0.04) 1px, transparent 1px, transparent 64px), repeating-linear-gradient(90deg, hsla(0,0%,100%,0.04) 0px, hsla(0,0%,100%,0.04) 1px, transparent 1px, transparent 64px)`,
            }}
          />
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-5 md:px-10 pt-8 md:pt-16 pb-6 md:pb-9">
          ...
          >
            ...
            {current.dataSource !== "BOM" && (
              <span className="byline text-white/70">Source · {current.dataSource ?? "BOM + models"}</span>
            )}
            <span className="byline text-white/70">Elev {u.elev(location.elevation)}{u.elevUnit}</span>
            {mountainWebsiteUrl && (
              <OfficialSiteLink
          ...
          <div className="mt-6 md:mt-8 flex items-center gap-2 text-white/60">
            <span className="byline text-white/60">Live conditions below</span>
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-10 pb-20 space-y-5 md:space-y-6 -mt-2">
        ...
        >
          ...
          {(() => {
            ...
            return (
              <div className={cn("grid gap-px rounded-2xl overflow-hidden bg-slate-100 border border-slate-100",
