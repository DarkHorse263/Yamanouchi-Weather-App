...
export function HomeFooter() {
  ...
  const linkCls = "font-semibold text-white hover:underline hover:underline-offset-2 transition-colors";
  const sep = <span className="text-white/70" aria-hidden>&middot;</span>;

  return (
    <footer className="relative z-10 border-t border-white/20 bg-[#0055FF]">
      <div className="mx-auto max-w-3xl px-5 py-5 md:py-6 text-center text-[13px] md:text-sm">
        <nav
          aria-label="footer"
        ...
        {sourcesOpen && (
          <ul className="mx-auto mt-3 inline-flex flex-wrap items-center justify-center gap-x-3 gap-y-1 rounded-lg bg-white/10 px-4 py-2 text-xs font-semibold text-white border border-white/30">
            <li><a className="hover:text-white" href="/snowy-mountains/sources">snowy mountains &middot; au</a></li>
            <li aria-hidden className="text-white/70">&middot;</li>
        ...
        {!fromAccount && (
          <div className="mt-3 inline-flex items-center gap-1 text-xs">
            <span className="font-semibold text-white mr-1">units</span>
            <div
              role="group"
              aria-label="units"
        ...
        <p className="mt-1.5 text-xs font-medium text-white max-w-xl mx-auto leading-relaxed">
