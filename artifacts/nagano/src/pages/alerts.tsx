  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">{t("Prefecture-wide snow alerts and storm tracking", "長野県全域の降雪アラートとストーム追跡")}</p>
      </div>
                ...
                <div className={`rounded-2xl p-5 border shadow-lg ${getAlertColor(alert.alertLevel)}`}>
                  ...
                  {alert.expectedSnow && (
                    <div className="inline-flex items-center gap-2 bg-black/20 px-3 py-1.5 rounded-lg mt-2">
                      <span className="text-sm font-bold uppercase opacity-80">{t("Expected:", "予想:")}</span>
            ...
            <Card key={storm.id} className="relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-2">
                <Badge variant={storm.status === 'active' ? 'destructive' : storm.status === 'incoming' ? 'primary' : 'outline'}>
                  {t(storm.status, storm.status === "active" ? "発生中" : storm.status === "incoming" ? "接近中" : "終了")}
                </Badge>
                <span className="text-xs font-bold text-muted-foreground">
                  {formatDate(storm.startDate)}
                </span>
              </div>
              ...
              <div className="flex gap-4 border-t border-border pt-3">
                ...
                {storm.peakSnow24h && (
                  <div>
                    <span className="text-xs font-bold uppercase text-muted-foreground block">{t("Peak 24h", "ピーク24h")}</span>
